...
function VehicleLoadPlanningMap({ locations, origin }: { locations: any[]; origin: { latitude: number; longitude: number; label: string } }) {
  ...
  return <section className="vehicle-load-planning-map panel"><header className="vehicle-load-planning-head"><div><h3>Mapa de la zona de entregas</h3><p className="muted">El zoom se centra en los clientes del día. La nave aparece como salida en el listado y las distancias se calculan desde Palencia.</p></div><strong>{valid.length}/{locations.length} ubicados</strong></header><div className="vehicle-load-planning-body"><div className="vehicle-load-planning-canvas"><iframe title="Mapa de la zona de entregas" src={mapSrc} loading="lazy" /><svg className="vehicle-load-planning-route" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><polyline points={routePoints} /></svg>{points.map((point, index) => { const projected = project(Number(point.latitude), Number(point.longitude)); return <span key={`${point.id || "delivery"}-${index}`} className="vehicle-load-planning-marker" style={{ left: `${projected.x}%`, top: `${projected.y}%` }} title={`${index + 1}. ${point.client_name || "Pedido"}`} >{index + 1}</span>; })}</div><ol className="vehicle-load-planning-stops"><li className="is-origin"><b>N</b><span><strong>Salida</strong><small>{origin.label}</small></span></li>{valid.map((item: any, index: number) => <li key={item.id}><b>{index + 1}</b><span><strong>{item.client_name || "Cliente sin nombre"}</strong><small>{item.opening_time && item.closing_time ? `Recepción ${String(item.opening_time).slice(0, 5)}–${String(item.closing_time).slice(0, 5)}` : "Horario pendiente"}{item.distance_km === null ? " · Distancia pendiente" : ` · ${String(item.distance_km).replace(".", ",")} km`}</small></span></li>)}</ol></div>{valid.length < locations.length && <p className="vehicle-load-planning-warning">{locations.length - valid.length} pedido{locations.length - valid.length === 1 ? " sin" : "s sin"} coordenadas. No se puede calcular su posición en la ruta hasta geolocalizarlo.</p>}</section>;
}
...
function VehicleOperationsPanel({ user, routeDate, vehicles, onReload }: { user: any; routeDate: string; vehicles: any[]; onReload: () => void }) {
  ...
  const [vehicleDraft, setVehicleDraft] = useState<any>({ name: "", plate: "", odometer_km: "", maintenance_interval_km: "30000", maintenance_interval_days: "180", next_maintenance_date: "" });
  const [fuelDraft, setFuelDraft] = useState<any>({ vehicle_id: "", fuel_date: routeDate, station: "", liters: "", amount: "", ticket_reference: "", odometer_km: "", notes: "" });
  const [maintenanceDraft, setMaintenanceDraft] = useState<any>({ vehicle_id: "", maintenance_date: routeDate, maintenance_km: "", maintenance_type: "Mantenimiento general", amount: "", next_due_km: "", next_due_date: "", notes: "" });
  const [tripSaving, setTripSaving] = useState<number | null>(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState("");
  ...
  const selectedVehicle = vehicles.find((vehicle: any) => Number(vehicle.id) === Number(selectedVehicleId));
  function chooseVehicle(id: any) {
    const value = String(id || "");
    setSelectedVehicleId(value);
    setFuelDraft((current: any) => ({ ...current, vehicle_id: value, odometer_km: vehicles.find((vehicle: any) => Number(vehicle.id) === Number(value))?.odometer_km || "" }));
    setMaintenanceDraft((current: any) => ({ ...current, vehicle_id: value, maintenance_km: vehicles.find((vehicle: any) => Number(vehicle.id) === Number(value))?.odometer_km || "" }));
  }
  async function saveVehicle(event: FormEvent) {
    event.preventDefault(); setSaving(true); setError(""); setMessage("");
    try { const response = await fetch("/api/vehicles", { method: "POST", headers: { "Content-Type": "application/json", "X-Actor": user?.username || "Usuario local" }, body: JSON.stringify(vehicleDraft) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || "No se pudo guardar el camión"); setVehicleFormOpen(false); setVehicleDraft({ name: "", plate: "", odometer_km: "", maintenance_interval_km: "30000", maintenance_interval_days: "180", next_maintenance_date: "" }); setMessage("Camión guardado correctamente."); onReload(); } catch (caught: any) { setError(caught?.message || "No se pudo guardar el camión."); } finally { setSaving(false); }
  }
  async function saveTrip(trip: any, draft: any) {
    setTripSaving(Number(trip.id)); setError(""); setMessage("");
    try { const response = await fetch(`/api/vehicle_trips/${trip.id}`, { method: "PUT", headers: { "Content-Type": "application/json", "X-Actor": user?.username || "Usuario local" }, body: JSON.stringify(draft) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || "No se pudieron guardar los kilómetros"); setTrips((current) => current.map((item) => Number(item.id) === Number(trip.id) ? body : item)); setMessage(`Kilómetros de ${trip.code} guardados.`); onReload(); } catch (caught: any) { setError(caught?.message || "No se pudieron guardar los kilómetros."); } finally { setTripSaving(null); }
  }
  async function saveFuel(event: FormEvent) {
    event.preventDefault(); setSaving(true); setError(""); setMessage("");
    try { const response = await fetch("/api/vehicle_refuels", { method: "POST", headers: { "Content-Type": "application/json", "X-Actor": user?.username || "Usuario local" }, body: JSON.stringify(fuelDraft) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || "No se pudo registrar el repostaje"); setFuelFormOpen(false); setRefuels((current) => [body, ...current]); setMessage("Repostaje registrado con su ticket e importe."); onReload(); } catch (caught: any) { setError(caught?.message || "No se pudo registrar el repostaje."); } finally { setSaving(false); }
  }
  ...
  return <section className="vehicle-operations panel">
    ...
    <div className="vehicle-fleet-grid">{vehicles.length ? vehicles.map((vehicle: any) => <button type="button" className={`vehicle-fleet-card${selectedVehicleId === String(vehicle.id) ? " is-selected" : ""}`} key={vehicle.id} onClick={() => chooseVehicle(vehicle.id)}><span className="vehicle-fleet-title"><b>{vehicle.plate || vehicle.name}</b><em className={vehicle.maintenance_alert ? "is-alert" : "is-ok"}>{vehicle.maintenance_status}</em></span><span><strong>{Number(vehicle.odometer_km || 0).toLocaleString("es-ES")} km</strong><small>kilometraje acumulado</small></span><span><strong>{vehicle.next_maintenance_km ? `${Number(vehicle.next_maintenance_km).toLocaleString("es-ES")} km` : "Sin límite de km"}</strong><small>próximo mantenimiento{vehicle.next_maintenance_date ? ` · ${formatSpanishDateValue(vehicle.next_maintenance_date, false)}` : ""}</small></span><span><strong>{Number(vehicle.total_fuel_liters || 0).toLocaleString("es-ES", { maximumFractionDigits: 1 })} l · {Number(vehicle.total_fuel_amount || 0).toLocaleString("es-ES", { style: "currency", currency: "EUR" })}</strong><small>gasoil acumulado</small></span>{vehicle.maintenance_alert && <small className="vehicle-fleet-alert">{vehicle.km_remaining !== null && vehicle.km_remaining <= 0 ? "Superado el límite de kilómetros" : vehicle.days_remaining !== null && vehicle.days_remaining <= 0 ? "Superada la fecha prevista" : `Revisar en ${Math.max(0, vehicle.km_remaining || 0).toLocaleString("es-ES")} km o ${Math.max(0, vehicle.days_remaining || 0)} días`}</small>}</button>) : <p className="empty-state">Todavía no hay camiones dados de alta. Añade el primero para poder asignarlo a una ruta.</p>}</div>
    {trips.length > 0 && <div className="vehicle-trip-list"><div className="vehicle-subhead"><b>Jornadas del {formatSpanishDateValue(routeDate, false)}</b><small>Introduce los kilómetros iniciales y finales para cerrar el día y actualizar el acumulado.</small></div>{trips.map((trip: any) => <div className="vehicle-trip-row" key={trip.id}><div><b>{trip.vehicle_plate || trip.vehicle_name}</b><strong>{trip.route_code || trip.code}</strong><small>{trip.driver || "Sin conductor"} · previsto {Number(trip.planned_distance_km || 0).toLocaleString("es-ES", { maximumFractionDigits: 1 })} km</small></div><label>Km inicial<input type="number" min="0" defaultValue={trip.start_km ?? ""} id={`trip-start-${trip.id}`} /></label><label>Km final<input type="number" min="0" defaultValue={trip.end_km ?? ""} id={`trip-end-${trip.id}`} /></label><span>{trip.distance_km === null || trip.distance_km === undefined ? "Sin cerrar" : `${Number(trip.distance_km).toLocaleString("es-ES", { maximumFractionDigits: 1 })} km reales`}</span><button type="button" className="button secondary" disabled={tripSaving === Number(trip.id)} onClick={() => void saveTrip(trip, { start_km: (document.getElementById(`trip-start-${trip.id}`) as HTMLInputElement)?.value, end_km: (document.getElementById(`trip-end-${trip.id}`) as HTMLInputElement)?.value, status: "Completada" })}>{tripSaving === Number(trip.id) ? "Guardando…" : "Guardar km"}</button></div>)}</div>}
...
function CollectiveLoadModal({ rows, lookups, dateFilter, actor, onClose, onDateFilterChange, embedded = false }: { rows: any[]; lookups: any; dateFilter: string; actor: string; onClose: () => void; onDateFilterChange?: (value: string) => void; embedded?: boolean }) {
  ...
  async function saveLine(line: any) {
    ...
    setEditingLineIds((current) => {
      ...
      delete nextEditing[String(line.id)];
      return nextEditing;
    });
    try {
          ...
          <div className="collective-load-list collective-load-simple-list">
            ...
            {displayLines.map((line: any) => {
              ...
              return <article className={`collective-load-simple-row${lineStateClass}`} key={line.id}>
                <div className="collective-load-simple-main"><strong className="collective-load-simple-location">{product?.warehouse_location ? warehouseLocationLabel(product.warehouse_location) : "Sin ubicación"}</strong><div className="collective-load-simple-product"><b>{product?.name || `Producto #${line.product_id}`}</b></div></div>
                <div className="collective-load-simple-check"><label className={`collective-load-simple-barcode collective-load-barcode-field barcode-${scannedStatus}`}><input aria-label={`Código de barras del producto ${product?.name || line.product_id}`} value={scannedCode} placeholder={expectedBarcode(line) || "Escanea o escribe el código"} disabled={validated && !editing} onChange={(event) => setBarcodeDrafts((current) => ({ ...current, [String(line.id)]: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") event.preventDefault(); }} /></label><label className="collective-load-simple-quantity"><span>Uds. (pedido: {requestedQuantity(line)})</span><input aria-label={`Cantidad preparada de ${product?.name || `producto ${line.product_id}`}. Pedido original: ${requestedQuantity(line)} unidades`} type="number" min="0" max={requestedQuantity(line)} step="any" value={drafts[String(line.id)] ?? String(defaultDraftQuantity(line))} disabled={validated && !editing} onChange={(event) => setDrafts((current) => ({ ...current, [String(line.id)]: event.target.value }))} /></label><div className="collective-load-simple-action">{validated && !editing ? <button type="button" className="row-action workflow" disabled={savingId !== null || incidentSaving} onClick={() => setEditingLineIds((current) => ({ ...current, [String(line.id)]: true }))}>Editar</button> : Number(drafts[String(line.id)] ?? defaultDraftQuantity(line)) < requestedQuantity(line) ? <button type="button" className="row-action danger" disabled={incidentSaving || savingId !== null} onClick={() => { setIncidentLineId(Number(line.id)); setIncidentText(""); setError(""); }}>{incident ? "Revisar" : "Incidencia"}</button> : <button type="button" className="row-action workflow" disabled={savingId !== null || incidentSaving} onClick={() => void saveLine(line)}>{savingId === line.id ? "Guardando…" : editing ? "Guardar" : "Validar"}</button>}</div></div>
              </article>;
            })}
          </div>
                 ...
                 const product = getProduct(line);
                   ...
                   const lineStateClass = barcodeMismatch ? " is-barcode-mismatch" : validated && !editing ? " is-validated" : incident ? " is-incident" : " is-pending";
                    return <div className={`collective-load-source-row${lineStateClass}`} key={line.id}>
                     ...
                     <label className="collective-load-prepared-field">Cantidad preparada<input type="number" min="0" max={requestedQuantity(line)} step="any" value={drafts[String(line.id)] ?? String(defaultDraftQuantity(line))} disabled={validated && !editing} onChange={(event) => setDrafts((current) => ({ ...current, [String(line.id)]: event.target.value }))} /></label>
                     <span className={`collective-load-status${validated && !editing ? " valid" : incident ? " incident" : " pending"}`}>{editing ? "Editando" : validated ? "Validada" : incident ? "Incidencia" : "Pendiente"}</span>
...
function GoodsReceiptForm({ lookups, actor, onCreated }: { lookups: any; actor: string; onCreated: (receipt: any) => void }) {
  ...
  function addProduct(product: any) {
    if (!product) return;
    setLines((current) => [...current, { product_id: String(product.id), product_name: product.name, sku: product.sku || "", expected_quantity: "0", received_quantity: "1", damaged_quantity: "0", substituted_quantity: "0", substitute_product_id: "", lot_code: "", expiry_date: "", unit_cost: String(Number(product.cost_price || 0)), status: "Correcta", notes: "", incident_description: "", attachments: [] }]);
    setProductSearch("");
    setMessage("");
  }
  ...
  return <div className="goods-receipt-editor">
    ...
    <section className="goods-receipt-lines"><div className="goods-receipt-lines-head"><div><b>Productos recibidos</b><small>Busca por nombre, SKU, marca o categoría y añade las líneas que correspondan. Puedes añadir el mismo producto varias veces para separar sus lotes.</small></div><span>{lines.length} añadidas</span></div>
...
function Manager({ active, user, onNavigate, assistantFormIntent, onAssistantFormConsumed, warehouseMode = false }: { active: string; user?: any; onNavigate?: (module: string, date?: string) => void; assistantFormIntent?: any; onAssistantFormConsumed?: () => void; warehouseMode?: boolean }) {
  ...
  const [preparationTraceabilitySavingId, setPreparationTraceabilitySavingId] = useState<number | null>(null);
                ...
                {previewLines.map((line: any) => {
                  ...
                  const displayLineStatus = lineIsComplete
                    ...
                    : line.preparation_status === "Incidencia"
                      ? "Incidencia"
                      : "Incompleto";
                  const displayLineStatusClass = displayLineStatus === "Pendiente de validar"
                    ? "pendiente"
