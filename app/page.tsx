          ...
          <div className="collective-load-list collective-load-simple-list">
            ...
            {displayLines.map((line: any) => {
              ...
              return <article className={`collective-load-simple-row${lineStateClass}`} key={line.id}>
                ...
                <div className="collective-load-simple-check"><label className={`collective-load-simple-barcode collective-load-barcode-field barcode-${scannedStatus}`}><input aria-label={`Código de barras del producto ${product?.name || line.product_id}`} value={scannedCode} placeholder={expectedBarcode(line) || "Escanea o escribe el código"} disabled={validated && !editing} onChange={(event) => setBarcodeDrafts((current) => ({ ...current, [String(line.id)]: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") event.preventDefault(); }} /></label><label className="collective-load-simple-quantity"><span>Uds.</span><input aria-label={`Cantidad preparada de ${product?.name || `producto ${line.product_id}`}`} type="number" min="0" max={requestedQuantity(line)} step="any" value={drafts[String(line.id)] ?? String(defaultDraftQuantity(line))} disabled={validated && !editing} onChange={(event) => setDrafts((current) => ({ ...current, [String(line.id)]: event.target.value }))} /></label><div className="collective-load-simple-action">{validated && !editing ? <button type="button" className="row-action workflow" disabled={savingId !== null || incidentSaving} onClick={() => setEditingLineIds((current) => ({ ...current, [String(line.id)]: true }))}>Editar</button> : Number(drafts[String(line.id)] ?? defaultDraftQuantity(line)) < requestedQuantity(line) ? <button type="button" className="row-action danger" disabled={incidentSaving || savingId !== null} onClick={() => { setIncidentLineId(Number(line.id)); setIncidentText(""); setError(""); }}>{incident ? "Revisar" : "Incidencia"}</button> : <button type="button" className="row-action workflow" disabled={savingId !== null || incidentSaving} onClick={() => void saveLine(line)}>{savingId === line.id ? "Guardando…" : editing ? "Guardar" : "Validar"}</button>}</div></div>
              </article>;
            })}
          </div>
