...
const insert = async (table, values, executor = db) => {
  const columns = await columnsFor(table);
  const entries = Object.entries(values).filter(([key, value]) => columns.has(key) && value !== undefined);
  const result = await executor.execute({ sql: `INSERT INTO ${table}(${entries.map(([key]) => key).join(",")}) VALUES(${entries.map(() => "?").join(",")})`, args: entries.map(([, value]) => value) });
